            ...
            try {
                ...
                if (! $token = JWTAuth::attempt($credentials)) {
                    ...
                    if($user instanceof User){
                        if($user->status == true and $user->user_type_id == 1){
                            $user_info = User::where('id', '=', $user->id)->with('user_type')->first();
...
//                            $user_log = $this->saveLogHistory($user);
                            return response()->json(['status'=>true, 'message'=> 'successfully authenticated',  'token'=>$token, 'user'=>$user_info,
                                'roles'=>$user_roles, 'user_permissions'=>$user_permission ],200);
                        }else{
...
//              $user_log ->last_login_at = Carbon::now()->toDateTimeString();
//              $user_log ->save();
    ...
    public function register(Request $request)
    {
        try {
            ...
            if($old_user instanceof User) {
                ...
                if($newUser->save()) {
                    $viewer_role = Role::where('id', '=', 1)->first();
